...
public class PacketIdPartitioner<K, V> implements StreamPartitioner<K, V> {

    @Override
    public Integer partition(String topic, K key, V value, int numPartitions) {


        if (key instanceof TimKey) {
            var timKey = (TimKey)key;
            String packetId = timKey.getPacketId();
            return packetId.hashCode() % numPartitions;
        } else {
            byte[] partitionBytes;
