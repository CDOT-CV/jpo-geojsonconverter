...
/**
 * Kafka Streams Topology builder for processing BSM messages from ODE BSM JSON -> BSM GeoJSON
 */
public class BsmTopology {

    private static final Logger logger = LoggerFactory.getLogger(BsmTopology.class);

    public static Topology build(String bsmOdeJsonTopic, String bsmProcessedJsonTopic,
            BsmJsonValidator bsmJsonValidator) {
        StreamsBuilder builder = new StreamsBuilder();

        // Stream for raw BSM messages
        KStream<Void, Bytes> rawOdeBsmStream = builder.stream(bsmOdeJsonTopic, Consumed.with(Serdes.Void(), // Raw topic
                                                                                                            // has no
                                                                                                            // key
        ...
        KStream<Void, DeserializedRawBsm> validatedOdeBsmStream = rawOdeBsmStream.mapValues((Void key, Bytes value) -> {
        ...
        processedJsonBsmStream.to(
                ...
                bsmProcessedJsonTopic, Produced.with(JsonSerdes.RsuLogKey(), JsonSerdes.ProcessedBsm(),
