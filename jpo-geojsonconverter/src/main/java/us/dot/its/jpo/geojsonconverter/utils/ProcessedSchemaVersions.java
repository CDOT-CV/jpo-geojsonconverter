package us.dot.its.jpo.geojsonconverter.utils;

/**
 * ProcessedSchemaVersions is a utility class that holds the schema versions for various processed message types. It
 * provides constants for the schema versions. Please update this class whenever a schema version changes or a new
 * processed message type is introduced.
 */
public class ProcessedSchemaVersions {
    public static final int PROCESSED_BSM_SCHEMA_VERSION = 2;
    public static final int PROCESSED_MAP_SCHEMA_VERSION = 2;
    public static final int PROCESSED_SPAT_SCHEMA_VERSION = 3;
    public static final int PROCESSED_PSM_SCHEMA_VERSION = 2;
    public static final int PROCESSED_RTCM_SCHEMA_VERSION = 1;
    public static final int PROCESSED_TIM_SCHEMA_VERSION = 1;
    public static final int PROCESSED_SSM_SCHEMA_VERSION = 1;
    public static final int PROCESSED_SRM_SCHEMA_VERSION = 1;
}
